...
All kinds of fancy parameters are passed in through argument. Use = to connect the key and the corresponding value. Use & to connect various keys. You can choose the parameters you want to fill in.
title：title
mb parameter: the traffic consumed in each test, the maximum limit is 4MB after testing. Parameter: &mb=4, default 3MB
iconfast, iconmid, and iconslow respectively correspond to the icons when the speed is measured at fast, medium and slow speeds.
colorlow, colormid, and colorhigh respectively correspond to the icon colors when the delay is low, medium, and high.
Configuration example: title=Frills are productivity&iconfast=bird&iconmid=hare&iconslow=tortoise&colorlow=#06D6A0&colormid=#FFD166&colorhigh=#EF476F

⚠️Don’t want variety? ?
You can directly use the most basic panel parameters, title, icon, icon-color
Configuration example: titile=don’t want to be fancy&icon=hare&icon-color=#CDCDCD
*/
...
!(async () => {
    ...
    title = `NetSpeed`;
    content = `Download speed: ${round(Math.abs(speed * 8))} Mbps [${round(
        Math.abs(speed, 2),
        1
    )} MB/s]\nTesting time: ${round(
        Math.abs(duration, 2),
        2
    )}s\nNetwork delay: ${pingt} ms\nExecution time: ${
