/**
 * The network request is encapsulated as Promise
...
function loadCarrierNames() {
    // Arrangement logic: the first three digits are the same -> the last two digits are the same operator -> the rest
    return {
        //Taiwan operator
        "466-11": "中華電信",
        "466-92": "中華電信",
...
//Get mobile phone operator information (call device information through built-in API)
...
function getIP() {
    ...
    if (!v4 && !v6) {
        info = [
            ...
            "Please refresh manually to obtain IP again",
        ];
    } else {
        ...
        if (v4?.primaryRouter && getSSID())
            info.push(`Router IP:${v4?.primaryRouter}`);
 ...
 * @param {*} retryInterval // Retry interval ms
 */
function getNetworkInfo(retryTimes = 5, retryInterval = 1000) {
    ...
    httpMethod
        ...
        .then((response) => {
            ...
            $done({
                ...
                content:
                    ...
                    `IP: ${info.query}\n` +
                    `ISP: ${info.isp}\n` +
                    `Location: ${getFlagEmoji(info.countryCode)} | ${
                        info.city
                    } - ${info.country}`,
        ...
        .catch((error) => {
            ...
            // Determine whether there is still a chance to retry
            if (retryTimes > 0) {
                logger.error(error);
                logger.log(`Retry after ${retryInterval}ms`);
                // retryInterval Execute the function again after time
                setTimeout(
...
/**
 * Main logic, program entry
 */
(() => {
    const retryTimes = 5;
    const retryInterval = 1000;
    // Surge The script timeout is set to 30s
    // In advance 500ms end process manually
    const surgeMaxTimeout = 29500;
    // Script timeout
    // retryTimes * 5000 Timeout for each network request (Surge network request timeout is 5s)
